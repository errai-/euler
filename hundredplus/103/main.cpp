...
void distributor(vec& prev, unsigned editor) {
    if (editor < amnt) {
        for (int i = -6; i <= 6; ++i) {
            vec curr(prev.begin(),prev.end());
            curr[editor] += i;
            distributor(curr, editor+1);
        }
    } else {
        ++neger;
        //seqs.insert( stringify(prev) );
        if (work_horse(prev) && vec_sum(prev) <= 255) {
            cout << "  " << vec_sum(prev) << "   ";
            cout << stringify( prev ) << endl;
        }
    }
}
...
int main(void) {
    ...
    cout << "Neger: " << neger << " " << seqs.size() << endl;
    return 0;
}
